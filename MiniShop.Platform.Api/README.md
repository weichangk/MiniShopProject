...
#!/bin/bash
echo "获取当前容器是否存在-----------------------------------------------------------------"
containerps=$(docker ps -f name=minishopplatformapi -q)
containerstop=$(docker ps -a -f name=minishopplatformapi -q)
...
echo "获取当前镜像是否存在-----------------------------------------------------------------"
dockerlist=$(docker images minishopplatformapi:latest -q)
...
docker build -t minishopplatformapi:$image_version .

echo "启动容器-------------------------------------------------"
docker run -d -p 15401:80 --restart=always -v /home/myvolumes/minishopplatformapi/appsettings.json:/app/appsettings.json -v /home/myvolumes/minishopplatformapi/log:/app/log --name minishopplatformapi minishopplatformapi:$image_version
